...
// ⚠️ ГЛАВНОЕ ИЗМЕНЕНИЕ: компонент должен быть асинхронным, а params — await
export default async function LessonPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; // ← Разворачиваем промис
  const lessonId = parseInt(id);
  const lesson = lessons[lessonId] || lessons[1];

  // Вся клиентская логика остаётся без изменений
  return <LessonClient lesson={lesson} lessonId={lessonId} />;
}

// Клиентский компонент (со всем состоянием)
function LessonClient({ lesson, lessonId }: { lesson: (typeof lessons)[1]; lessonId: number }) {
